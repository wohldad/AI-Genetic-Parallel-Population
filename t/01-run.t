...
can_ok( 'AI::Genetic::Parallel::Population', 'new', 'individuals', 'fittest', 'weakest', 'best', 'worst', 'elites', 'keep', 'send_reaper' );
...
foreach my $idx ( 0 .. 7 ) {
    is( $pop->individuals->[$idx]->fitness($idx), $idx, "set fitness for DNA " . $pop->individuals->[$idx]->dna . " to $idx" );
}


#  check to see that all the fitnesses are there
foreach my $idx ( 0 .. 7 ) {
    is( $pop->individuals->[$idx]->fitness, $idx, "got fitness for DNA " . $pop->individuals->[$idx]->dna . " as $idx" );
}


#  set one of the DNA's as elite
is( $pop->individuals->[0]->elite( 1 ), 1, "set DNA 000 to elite" );
...
is( scalar @{$pop->individuals}, '4',               'kept best 4'                       );


#  test the grimm reaper
$pop->individuals->[0]->death_wish(1);
isa_ok( $pop->send_reaper,           'AI::Genetic::Parallel::Population'     );
is( scalar @{$pop->individuals}, '3', 'we killed on member of the populaiton' );
