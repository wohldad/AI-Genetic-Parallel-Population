...
can_ok( 'AI::Genetic::Parallel::Population', 'new', 'members', 'fittest', 'weakest', 'best', 'worst', 'elites', 'keep', 'send_reaper' );
...
foreach my $individual ( @{ $pop->members } ) {
...
foreach my $idx ( 0 .. 7 ) {
    is( $pop->members->[$idx]->fitness($idx), $idx, "set fitness for DNA " . $pop->members->[$idx]->dna . " to $idx" );
}


#  check to see that all the fitnesses are there
foreach my $idx ( 0 .. 7 ) {
    is( $pop->members->[$idx]->fitness, $idx, "got fitness for DNA " . $pop->members->[$idx]->dna . " as $idx" );
}


#  set one of the DNA's as elite
is( $pop->members->[0]->elite( 1 ), 1, "set DNA 000 to elite" );
...
is( scalar @{$pop->members}, '4',               'kept best 4'                       );


#  test the grimm reaper
$pop->members->[0]->death_wish(1);
isa_ok( $pop->send_reaper,           'AI::Genetic::Parallel::Population'     );
is( scalar @{$pop->members}, '3', 'we killed on member of the populaiton' );
